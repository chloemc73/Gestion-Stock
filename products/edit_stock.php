...
if (!$stock) {
	$session->msg("d", "ID de la commande manquante.");
...
if (isset($_POST['edit_stock'])) {
	...
	if ( $quantity != $stock['quantity'] ) {
		//Il y a eu une augmentation de la quantité.
		if ( $quantity > $stock['quantity'] ) {
		...
		// Il y a eu une diminution de la quantité.
	...
	if (empty($errors)) {
		...
		if ($result && $db->affected_rows() === 1) {
			...
			$session->msg("s", "Mis à jour avec succès.");
			redirect('../products/stock.php', false);
		} else {
			$session->msg("d", "Mis à jour avec succès.");
     ...
     <div class="panel panel-default">
       <div class="panel-heading">
         <strong>
           ...
           <span>Modifier <?php echo $stock['product_id'];?></span>
        </strong>
       </div>
       <div class="panel-body">
         <form method="post" action="">
		   ...
		   <div class="input-group">
			 ...
			 <input type="number" class="form-control" name="quantity" value="<?php echo $stock['quantity'] ;?>" 
			 placeholder="Quantité Produit ">
		  </div>
           </div>

           <div class="form-group">
               <input type="text" class="form-control" name="comments" value="<?php echo $stock['comments'];?>"
			    placeholder="Notes">
           </div>

           <button type="submit" name="edit_stock" class="btn btn-primary">Modifier le stock</button>
       </form>
       </div>
     </div>
 </div>
