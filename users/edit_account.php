...
if (isset($_POST['submit'])) {
	...
	if ($photo->process_user($user_id)) {
		$session->msg('s', 'La photo é été mise à jour.');
		redirect('../users/edit_account.php');
	} else {
...
?>
<?php
//Mise à jour des autres information de l'utilisateur
if (isset($_POST['update'])) {
	...
	if (empty($errors)) {
		...
		if ($result && $db->affected_rows() === 1) {
			...
			$session->msg('d', ' Désolé, la mise à jour a échoué!');
			redirect('../users/edit_account.php', false);
		}
	} else {
            ...
            <div class="col-md-8">
              ...
              <div class="form-group">
                <input type="hidden" name="user_id" value="<?php echo $user['id'];?>">
                 <button type="submit" name="submit" class="btn btn-warning" style="background-color:rgb(255, 143, 99)"
                 >Changer</button>
              </div>
             </form>
            </div>
      ...
      <div class="panel-body">
          <form method="post" action="../users/edit_account.php?id=<?php echo (int)$user['id'];?>" class="clearfix">
            ...
            <div class="form-group clearfix">
                    <a href="../users/change_password.php" title="change password" class="btn btn-danger pull-right"
